...
Normal verification additionally derives every live receipt ID from canonical JSON, checks raw response artifacts against the ledger, and binds both `panel.attempts` and anonymized `panel.results` records—including outer seat, role, status, and response—to the same three receipts. It fixes the ten-call stage/seat/schema/provider/requested-model/actual-model/route topology, recomputes usage/cost totals, binds synthesis text to both gates, validates gate votes and the execution handoff, and cross-checks harness summaries and curated commitments. The scanner covers the verifier source itself as part of the complete payload tree. The mutation suite changes semantic evidence, performs a coordinated Grok-model receipt rewrite, injects a credential into the verifier source, and then refreshes file hashes, demonstrating that checksum regeneration cannot hide those mutations.
